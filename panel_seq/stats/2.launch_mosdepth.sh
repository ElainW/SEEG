...
bed_int=/n/groups/walsh/indData/elain/scripts/panel_seq_input/FCDHME-122021-2_1_Covered.b37.bed
CALC_DEPTH() {
	input=$1
	prefix=$2
	outdir=${input%.txt}
	...
	while read LINE; do
		...
		sbatch -N 1 -c 4 --mem 3G -J ${outdir}_$outprefix -t $3 -p short -o $outdir/$outprefix/${prefix}_mosdepth.log --mail-user=yilanwang@g.harvard.edu --mail-type=FAIL,END --wrap="mosdepth -t 4 -b $bed_int $outdir/$outprefix/$prefix $in_bam; \
...
CALC_DEPTH_RE() {
	...
	while read LINE; do
		...
		if [[ $outprefix=="KMTS51" ]]; then
			...
			sbatch -N 1 -c 4 --mem 3G -J ${outdir}_$outprefix -t $3 -p short -o $outdir/$outprefix/${prefix}_mosdepth.log --mail-user=yilanwang@g.harvard.edu --mail-type=FAIL --wrap="mosdepth -t 4 -b $bed_int $outdir/$outprefix/$prefix $in_bam; \
															gzip -d $outdir/$outprefix/$prefix.per-base.bed.gz; \
															bash calc_avg_cov.sh $outdir/$outprefix/$prefix $bed_int"
		fi
	done < $input
...
GET_DEPTH() {
	...
	prefix=$2
	outdir=${input%.txt}
	count_file=${outdir}_depth_XI2.txt
	printf "" > $count_file
	while read LINE; do
		IFS=, read outprefix in_bam <<< $LINE
		if [ -f $outdir/$outprefix/$prefix.per-base.bed ]; then
