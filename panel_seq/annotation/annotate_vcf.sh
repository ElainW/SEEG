...
INDIR=$ROOT/panel_seq_output/merged_vcf
...
ANNOTATE_SNV() {
	...
	unset PYTHONPATH
	sbatch -N 1 -c 1 -J ${prefix}_spliceai -o $OUTDIR/${prefix}_filtered_spliceai.log --mem=5G -t 0-03:00 -p short --mail-user=yilanwang@g.harvard.edu --mail-type=END,FAIL --wrap="spliceai -I $INDIR/intermediate/${prefix}_filtered_splice.vcf -O $OUTDIR/${prefix}_filtered_splice_spliceai.vcf -R /n/data1/bch/genetics/lee/reference/hg19/ucsc.hg19.fasta -A grch37; python3 filter_spliceai.py $OUTDIR/${prefix}_filtered_splice_spliceai.vcf 0.8"
}
